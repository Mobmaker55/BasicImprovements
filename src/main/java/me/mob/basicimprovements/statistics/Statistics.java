...
public enum Statistics {
    ...
    public String get(UUID uuid, Player invoker) {
        ...
        if (divisor == 0) {
            Pair<Double, String> time = timeRounding(statistic);
            finalStatistic = time.getKey();
            theUnits = time.getValue();
        } else {
            finalStatistic = StatisticData.round(statistic/divisor, 2);
            theUnits = units;
        }
        if (invoker == player) {
    ...
    public Pair<Double, String> timeRounding(long value) {
        double valueSec;
        double valueMin;
        double valueHr;
        String unit = "seconds";
        double valueReturn = value;
        valueSec = StatisticData.round(value / 20.0, 0);
        if (valueSec >= 60) {
            valueMin = StatisticData.round(valueSec / 60, 1);
            System.out.println(valueMin);
            unit = "minutes";
            if (valueMin >= 120) {
                valueHr = StatisticData.round(valueMin / 60, 1);
                System.out.println(valueHr);
                unit = "hours";
                if (valueHr >= 120) {
                    valueReturn = StatisticData.round(valueHr / 24, 2);
